...
import { X, User, Shield, Calculator, Heart, FileText, Calendar, Mail, Phone, RefreshCw, DollarSign } from 'lucide-react'
import { getFamilyMembers, getPolicies, getQuotes, getNeedAnalysis, addFamilyMember, updateFamilyMember, removeFamilyMember } from '../../utils/persons.js'
      ...
      <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeSection === 'quotes' && (
            <>
              <h4 className="text-sm font-bold text-slate-800 mb-3">Quotes for {member.firstName}</h4>
              {loadingQuotes ? (
                <div className="flex items-center gap-2 text-sm text-slate-500"><RefreshCw size={14} className="animate-spin" /> Loading...</div>
              ) : quotes.length === 0 ? (
                      ...
                      {quotes.map((q) => {
                        ...
                        return (
                          <tr key={q.id}>
                            ...
                            <td className="py-2 text-slate-600">{q.createdAt ? new Date(q.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'N/A'}</td>
                          </tr>
                        )
                      })}
                    </tbody>
          ...
          {activeSection === 'analysis' && (
            <>
              <h4 className="text-sm font-bold text-slate-800 mb-3">Need Analysis for {member.firstName}</h4>
              {loadingNA ? (
                <div className="flex items-center gap-2 text-sm text-slate-500"><RefreshCw size={14} className="animate-spin" /> Loading...</div>
              ) : needAnalysis ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {(needAnalysis.annualIncomePrimary || needAnalysis.annualIncomeSpouse) && (
                      <div className="p-3 bg-slate-50 rounded-lg">
                        ...
                        <p className="text-sm font-semibold mt-0.5">
                          ...
                          {needAnalysis.annualIncomeSpouse ? ` / CHF ${Number(needAnalysis.annualIncomeSpouse).toLocaleString()}` : ''}
                        </p>
                      </div>
                    )}
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-700">
                    Full need analysis can be accessed from the Need Analysis tab.
                  </div>
                </div>
              ) : (
