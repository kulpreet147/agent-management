...
export default function PersonCreate() {
  ...
  const { selectedPerson, getPersonLoading, saving } = useSelector((s) => s.person)
  ...
  }, [dispatch, isEdit, personId])

  useEffect(() => {
    if (isEdit && selectedPerson) {
      setPriority(selectedPerson.leadPriority ? selectedPerson.leadPriority.toUpperCase() : '')
      setPhase(selectedPerson.phase || 'lead')
      if (selectedPerson.assignments?.length) {
        setAgents(
          selectedPerson.assignments.map((a) => ({
            agentId: a.agentId || a.id,
            agentUuid: a.agentUuid || a.agentId || a.id,
            name: a.agentName || a.name || 'Agent',
            role: a.role || 'Agent',
            split: a.commissionShare || 0,
            img: a.img || '',
          }))
        )
      }
  ...
  const handleSubmit = async (e) => {
    ...
    PRODUCTS.forEach((p) => {
      productInterest[p] = formData.getAll('productInterest').includes(p)
    })
    ...
    const payload = {
      ...
      productInterest:
        Object.values(productInterest).some(Boolean) ? productInterest : undefined,
    ...
    try {
      if (isEdit) {
        await dispatch(updatePersonAsync({ id: personId, data: payload })).unwrap()
        navigate(`/admin/persons/${personId}`)
      } else {
        const result = await dispatch(createPersonAsync(payload)).unwrap()
        const newId = result?.id || result?.personId || result?._id
        navigate(newId ? `/admin/persons/${newId}` : '/admin/persons')
      }
    } catch (err) {
      alert(err || 'Failed to save person.')
    }
  }

  if (isEdit && getPersonLoading) {
    return (
      <div className="flex-1 flex items-center justify-center py-24">
        <Loader2 size={32} className="animate-spin text-blue-600" />
        <span className="ml-3 text-slate-600 text-sm font-medium">Loading person…</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col items-center">
      <div className="w-full max-w-[900px] mb-6">
        <button
          type="button"
          onClick={() => navigate(isEdit ? `/admin/persons/${personId}` : '/admin/persons')}
          className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors"
        >
          <ArrowLeft size={18} />
          {isEdit ? 'Back to Person' : 'Back to Persons'}
        </button>
      </div>
      <form
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-lg border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all ${
                  fieldErrors.firstName ? 'border-red-500 bg-red-50' : 'border-slate-300'
                }`}
                placeholder="Enter first name"
                type="text"
              />
              {fieldErrors.firstName && (
                <p className="text-xs text-red-600 mt-0.5">{fieldErrors.firstName}</p>
              )}
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-lg border focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all ${
                  fieldErrors.lastName ? 'border-red-500 bg-red-50' : 'border-slate-300'
                }`}
                placeholder="Enter last name"
                type="text"
              />
              {fieldErrors.lastName && (
                <p className="text-xs text-red-600 mt-0.5">{fieldErrors.lastName}</p>
              )}
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                name="dateOfBirth"
                defaultValue={selectedPerson?.dateOfBirth?.slice(0, 10) || ''}
                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                type="date"
                max={today}
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all bg-white"
              >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                defaultValue={selectedPerson?.residencyStatus || 'Citizen'}
                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all bg-white"
              >
            ...
            <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-lg">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  name="smokingStatus"
                  defaultChecked={selectedPerson?.smokingStatus || false}
            ...
            <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-lg">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  name="alcoholStatus"
                  defaultChecked={selectedPerson?.alcoholStatus || false}
                  className="sr-only peer"
                  type="checkbox"
                />
                <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600" />
            ...
            <div className="col-span-2 flex flex-col gap-1.5">
              ...
              <input
                name="healthIssues"
                defaultValue={selectedPerson?.healthIssues || ''}
                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
                placeholder="Specify if any (e.g. Hypertension, Diabetes)"
                type="text"
              />
            </div>
            <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                name="employer"
                defaultValue={selectedPerson?.employer || ''}
                className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"
          ...
          <div className="space-y-6">
                ...
                {PRODUCTS.map((product) => (
                  ...
                  >
                    <input
                      ...
                      value={product}
                      defaultChecked={selectedPerson?.productInterest?.[product] || false}
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  name="leadSource"
                  defaultValue={selectedPerson?.leadSource || ''}
                  className="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all bg-white"
                >
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-slate-500 tracking-wider">PHASE</label>
              <select
                value={phase}
      ...
      <footer className="fixed bottom-0 left-[280px] right-0 bg-white border-t border-slate-200 px-8 py-4 flex items-center justify-between shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50">
        <div className="flex items-center gap-4 text-slate-500">
          <Info size={18} />
          <p className="text-sm">
            {isEdit ? `Editing ${selectedPerson?.firstName || ''} ${selectedPerson?.lastName || ''}` : 'Person ID will be automatically generated upon saving.'}
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
