...
export default function AgentPersonDetail() {
  const navigate = useNavigate()
  const dispatch = useDispatch()
  const { personId } = useParams()
  const session = auth.get()
  const agentName = session?.name || 'Agent'
  const sidebarInitials = agentName
    .split(' ')
    .map((p) => p[0])
    .join('')
    .slice(0, 2)
  ...
  useEffect(() => {
    if (!personId) {
      navigate('/agent/persons', { replace: true })
      return
    }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#eef3f8]">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Loading person details...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-[#eef3f8]">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate('/agent/persons')}
  ...
  return (
    <div className="min-h-screen bg-[#eef3f8] text-slate-950 max-w-[2200px] mx-auto">
      <div className="flex h-screen overflow-hidden">
        <AgentSidebar agentName={agentName} initials={sidebarInitials} />
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <h2 className="text-xl font-bold flex items-center gap-3">
                    {fullName}
                    <span className={`px-3 py-0.5 rounded-full text-xs font-bold uppercase border ${statusColors[person.status] || 'bg-slate-100 text-slate-600'}`}>
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-bold text-blue-700">
                    {person.personId || person.id?.slice(0, 8)}
                  </p>
                </div>
              </div>
                  ...
                  {tabs.map((tab, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`pb-3 text-sm font-semibold transition-colors whitespace-nowrap flex items-center gap-1.5 ${
                          isActive
                  ...
                  {activeTab === 0 && (
                    <>
                      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            ...
                            <p className="text-sm text-slate-800">{person.phone || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Email</p>
                            <p className="text-sm text-slate-800 break-all">{person.email || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Date of Birth</p>
                            <p className="text-sm text-slate-800">{person.dateOfBirth ? formatFullDate(person.dateOfBirth) : 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Address</p>
                            <p className="text-sm text-slate-800">{person.address || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Marital Status</p>
                            <p className="text-sm text-slate-800">{person.maritalStatus || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Residency</p>
                            <p className="text-sm text-slate-800">{person.residencyStatus || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Occupation</p>
                            <p className="text-sm text-slate-800">{person.occupation || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-xs text-slate-500 font-bold uppercase mb-1">Employer</p>
                            <p className="text-sm text-slate-800">{person.employer || 'N/A'}</p>
                          </div>
                          {person.smoking !== undefined && (
                            <div>
                              <p className="text-xs text-slate-500 font-bold uppercase mb-1">Smoking</p>
                              <p className="text-sm text-slate-800">{person.smoking ? 'Yes' : 'No'}</p>
                            </div>
                          )}
                          {person.alcohol !== undefined && (
                            <div>
                              <p className="text-xs text-slate-500 font-bold uppercase mb-1">Alcohol</p>
                              <p className="text-sm text-slate-800">{person.alcohol ? 'Yes' : 'No'}</p>
                            </div>
                          )}
                          {person.healthIssues && (
                            <div className="col-span-2">
                              <p className="text-xs text-slate-500 font-bold uppercase mb-1">Health Issues</p>
                              <p className="text-sm text-slate-800">{person.healthIssues}</p>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                          <p className="text-[10px] font-bold text-slate-400 uppercase">Total Policies</p>
                          <p className="text-xl font-bold text-emerald-700 mt-1">
                            {policies.length > 0 ? `${policies.length} (${activePolicies} active)` : 'None'}
                          </p>
                        </div>
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                          <p className="text-[10px] font-bold text-slate-400 uppercase">Family Members</p>
                          <p className="text-xl font-bold text-violet-700 mt-1">{familyMembers.length}</p>
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {activeTab === 4 && (
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <button
                          onClick={() => navigate(`/agent/persons/${personId}/need-analysis`)}
                    ...
                    <div className="space-y-2">
                      ...
                      {person.address && (
                        <div className="flex items-center gap-3 p-2.5 rounded-lg bg-slate-50">
                          ...
                          <span className="text-sm truncate">{person.address}</span>
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {showFollowUpModal && (
        ...
        >
          <div className="relative bg-white w-full max-w-[500px] rounded-xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="text-base font-semibold text-slate-900">Schedule Follow-Up</h3>
              <button
                type="button"
      ...
      {showNoteModal && (
        ...
        >
          <div className="relative bg-white w-full max-w-[480px] rounded-xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
      ...
      {showFamilyModal && (
        ...
        >
          <div className="relative bg-white w-full max-w-[520px] rounded-xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
      ...
      {showPolicyModal && (
        ...
        >
          <div className="relative bg-white w-full max-w-[560px] rounded-xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="text-base font-semibold text-slate-900">Add Policy</h3>
              <button
                type="button"
